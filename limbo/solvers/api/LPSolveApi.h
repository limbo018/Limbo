...
SolverProperty LPSolveLinearApi<T, V>::operator()(LPSolveLinearApi<T, V>::parameter_type* param)
{
	...
	set_lp_name(m_lpModel, "LPSolveLinearApi");
	// set verbose level 
    ...
    {
        variable_type var (i);
        limboAssertMsg(set_bounds(m_lpModel, i, m_model->variableLowerBound(var), m_model->variableUpperBound(var)), "failed to set bounds of variable for LP");
		limboAssertMsg(set_col_name(m_lpModel, i, m_model->variableName(var).c_str()), "failed to set name of variable %s", m_model->variableName(var).c_str());
        switch (m_model->variableNumericType())
        {
            case CONTINUOUS:
                break; 
            case BINARY: 
                limboAssertMsg(set_binary(m_lpModel, i, TRUE), "failed to set binary variable for LP"); 
                break;
            case INTEGER:
                limboAssertMsg(set_int(m_lpModel, i, TRUE), "failed to set integer variable for LP"); 
                break;
            default:
    ...
    for (unsigned int i = 0, ie = m_model->constraints().size(); i < ie; ++i)
    {
        ...
        limboAssertMsg(set_rowex(m_lpModel, i, vIdx.size(), &vValue[0], &vIdx[0]), "failed to set constraint for LP");
        switch (constr.sense())
        {
            case '>':
                limboAssertMsg(set_constr_type(m_lpModel, i, GE), "failed to set constraint type for LP");
                break;
            case '<':
                limboAssertMsg(set_constr_type(m_lpModel, i, LE), "failed to set constraint type for LP");
                break;
            case '=':
                limboAssertMsg(set_constr_type(m_lpModel, i, EQ), "failed to set constraint type for LP");
                break;
            default:
                limboAssertMsg(0, "unknown sense");
        }
        limboAssertMsg(set_rh(m_lpModel, i, constr.rightHandSide()), "failed to set constraint right-hand side for LP"); 
        limboAssertMsg(set_row_name(m_lpModel, i, m_model->constraintName(constr).c_str()), "failed to set constraint name %s for LP", m_model->constraintName(constr).c_str()); 
    }
...
#ifdef DEBUG_LPSOLVEAPI
    write_lp(m_lpModel, (char*)"problem.sol"); 
#endif
