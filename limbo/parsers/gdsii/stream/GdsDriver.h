...
void GdsDriver::general_cbk(string const& ascii_record_type, string const&, ContainerType const& vData)
{
	...
	{
		m_current = "BOUNDARY";
		limboAssertMsg(!m_lib.vCell.empty(), "%s block must be in a BGNSTR block", ascii_record_type.c_str());
	...
	else if (ascii_record_type == "XY")
	{
		if (m_current == "BOUNDARY")
		{
			limboAssertMsg((vData.size() % 2) == 0 && vData.size() > 4, "invalid size of data array: %lu", vData.size());
		...
		{
			limboAssertMsg(vData.size() == 2, "invalid size of data array for %s: %lu",
					m_current.c_str(), vData.size());
		...
		else limboAssertMsg(0, "record XY should only appear in BOUNDARY, BOX, TEXT, SREF");
	}
	else if (ascii_record_type == "STRING")
	{
		limboAssertMsg(m_current == "TEXT", "record type STRING must appear in TEXT block rather than %s", m_current.c_str());
		m_lib.vCell.back().vText.back().content.assign(vData.begin(), vData.end());
	}
	else if (ascii_record_type == "ENDEL")
	{
		limboAssertMsg(m_current == "BOUNDARY" || m_current == "TEXT"
				|| m_current == "SREF", 
	...
	{
		limboAssertMsg(m_current == "CELL", "BGNSTR and ENDSTR should be in pair");
		m_current = "LIBRARY"; // go back to upper 
	}
	else if (ascii_record_type == "ENDLIB")
	{
		limboAssertMsg(m_current == "LIBRARY", "BGNLIB and ENDLIB should be in pair");
		m_current = "HEADER"; 
		// call db 
		m_db.add_gds_lib(m_lib);
		m_lib.reset();
	}
	else limboAssertMsg(0, "unsupported record type: %s", ascii_record_type.c_str());
}
